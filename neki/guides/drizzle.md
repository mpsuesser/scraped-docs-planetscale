---
url: https://planetscale.com/docs/neki/guides/drizzle
title: "Drizzle"
description: ""
...
Drizzle connects to Neki through the `node-postgres` driver.
...
Create a `pg` pool and pass it to Drizzle:
...
This example uses top-level `await`. Set `"type": "module"` in `package.json`, or wrap the query in an async `main()` function.
