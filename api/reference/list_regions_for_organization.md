...
Call this before `create_database` to discover valid `region` slugs. Pick a region where `enabled` is `true` and where the field matching your engine (`mysql_supported` or `postgresql_supported`) is `true`. The entry with `current_default: true` is the organization default used when `region` is omitted.
