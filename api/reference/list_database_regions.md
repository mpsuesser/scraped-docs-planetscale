...
Results are limited to regions on the same cloud provider as the database's default branch. Use the `slug` of a region where `enabled` is `true` as the `region` parameter when creating branches; if omitted, the database's own region is used.
