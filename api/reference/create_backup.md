---
url: https://planetscale.com/docs/api/reference/create_backup
title: "Create_backup"
description: ""
...
Use `get_backup` to poll the current state of a backup.

To restore a backup, call `create_branch` with the `backup_id` and a required `cluster_size` (discover valid sizes via `list_cluster_size_skus`).
