...
After installing, load skill **`planetscale-pscale-cli-automation`** for CLI conventions and **`planetscale-safe-orchestrator`** when the user asks for a full PlanetScale assessment. See [github.com/planetscale/skills](https://github.com/planetscale/skills).
...
OAuth triggers automatically on first PlanetScale MCP tool use. See [PlanetScale MCP server](../mcp-server.md) for details.
...
Add the hosted MCP server to your Zed settings, using the URL above. See [PlanetScale MCP server](../mcp-server.md) for the current setup steps.
...
Do not edit project `AGENTS.md` without user approval. See skill `planetscale-mcp-agent-operating-model` in [planetscale/skills](https://github.com/planetscale/skills).
...
| MCP setup | [PlanetScale MCP server](../mcp-server.md) |
| AI & automation overview | [AI-enhanced development](../ai-tooling.md) |
